#!/usr/bin/env python3
"""
bench_devops.py — benchmarki wydajnościowe dla ishkarim_devops.

Mierzy:
  1. Czas importu modułu
  2. Czas load_knowledge_index()
  3. Przepustowość extract_python_blocks()
  4. Testy specyficzne dla obszaru devops

Uruchomienie:
    python projects/ishkarim-devops/benchmarks/bench_devops.py
    python projects/ishkarim-devops/benchmarks/bench_devops.py --quick
"""
...
def bench_load_index(root: str, n: int = 3) -> tuple[float, int]:
    ...
    for _ in range(n):
        t0 = time.perf_counter()
        docs = ishkarim_devops.load_knowledge_index(root=root)
        times.append(time.perf_counter() - t0)
        count = len(docs)
    times.sort()
...
def bench_extract_blocks(root: str, n_files: int = 20) -> tuple[float, int]:
    ...
    from ishkarim_devops.utils import read_work_md, extract_python_blocks
    import ishkarim_devops
    docs = ishkarim_devops.load_knowledge_index(root=root)
    sample = docs[:n_files]
...
def run_area_benchmarks(root: str, quick: bool) -> list[tuple[str, str, str]]:
    ...
    import importlib
    mod = importlib.import_module("ishkarim_devops")
    docs = mod.load_knowledge_index(root=root)
    n = min(len(docs), 20 if not quick else 5)

    from ishkarim_devops.utils import extract_tags, read_work_md
    t0 = time.perf_counter()
    for doc in docs[:n]:
...
def main():
    parser = argparse.ArgumentParser(description="Benchmarki dla ishkarim-devops")
    ...
    print(f"\n============================================================")
    print(f"  Benchmark: ishkarim-devops")
    print(f"  Root: {root}")
