"""
test_devops_domain.py — testy domenowe dla ishkarim_devops.
...
from ishkarim_devops import load_knowledge_index


@pytest.fixture
def repo_root():
    """Zwraca ścieżkę do katalogu głównego repozytorium Ishkarim."""
    root = Path(__file__).resolve().parents[3]  # projects/ishkarim-devops/tests/ → repo root
...
class TestRealData:
    ...
    def test_devops_docs_present(self, repo_root):
        docs = load_knowledge_index(root=str(repo_root))
        assert len(docs) >= 30

    def test_devops_self_healing_topics(self, repo_root):
        docs = load_knowledge_index(root=str(repo_root))
        healing = [d for d in docs if "heal" in d["name"].lower() or "naprawi" in d["name"].lower()]
        assert len(healing) > 0, "Brak katalogów self-healing"
...
class TestModuleIntegrity:
    ...
    def test_version_format(self):
        assert re.match(r"\d+\.\d+\.\d+", ishkarim_devops.__version__)

    def test_modules_all_strings(self):
        for name in ishkarim_devops.MODULES:
    ...
    def test_work_md_readable(self, repo_root):
        ...
        from ishkarim_devops.utils import read_work_md
        docs = load_knowledge_index(root=str(repo_root))
        readable = 0
        for doc in docs[:20]:
