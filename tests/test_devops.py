"""
test_devops.py — testy dla modułu ishkarim_devops.
"""
import pytest
...
class TestImport:
    ...
    def test_import_main(self):
        ...
        assert ishkarim_devops.__version__ == "0.1.0"

    def test_area(self):
        import ishkarim_devops
        assert ishkarim_devops.__area__ == "devops"

    def test_modules_list_nonempty(self):
        import ishkarim_devops
        assert len(ishkarim_devops.MODULES) > 0

    def test_submodule_imports(self):
        from ishkarim_devops import utils
...
class TestKnowledgeIndex:
    ...
    def test_returns_list(self, tmp_path):
        import ishkarim_devops
        result = ishkarim_devops.load_knowledge_index(root=str(tmp_path))
        assert isinstance(result, list)

    def test_record_keys(self, tmp_path):
        import os, ishkarim_devops
        # create a minimal fake source dir
        src_dir = tmp_path / ishkarim_devops.MODULES[0]
...
class TestUtils:
    ...
    def test_extract_python_blocks(self):
        from ishkarim_devops.utils import extract_python_blocks
        md = """```python
def greet():
    return "hello world"
